...
export default function AccountAccessDialog({ open, initialTab = "login", onClose, selectedService }) {
  ...
  if (!open) return null;
  const submitPreview = (event) => { event.preventDefault(); navigate("/add-property", { state: { previewNotice: true, selectedService } }); onClose(); };
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center overflow-y-auto bg-slate-800/55 p-4 py-8" role="dialog" aria-modal="true" aria-label="TRELPNG account access" data-testid="account-access-dialog">
    ...
    <div className="relative z-10 w-full max-w-[600px] rounded-[22px] bg-white px-9 py-8 shadow-2xl sm:px-10">
      ...
      <form onSubmit={submitPreview} className="mt-5 space-y-3">
        ...
        {tab === "login" ? <>
          <input required placeholder="Mobile number or email address" className="h-14 w-full rounded-lg border border-sky-200 px-4 text-sm outline-none focus:border-[#0398FC] focus:ring-2 focus:ring-sky-100" />
          ...
          <button type="submit" className="h-14 w-full rounded-full bg-[#0398FC] text-base font-semibold text-black">Login</button>
