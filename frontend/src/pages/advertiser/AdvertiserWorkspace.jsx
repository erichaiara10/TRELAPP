...
const steps=[["Property Details","/advertiser/add-property"],["Location & Identification","/advertiser/add-property/location"],["Features","/advertiser/add-property/features"],["Photos & Documents","/advertiser/add-property/photos"],["Review & Submit","/advertiser/add-property/review"]];
function Stepper({active}) { const navigate=useNavigate(); const flow=useDraft(); const go=async(index,path)=>{if(index===active)return;const ok=await flow.save(Math.max(1,active+1),true);if(ok)navigate(path);};return <div className="adv-stepper">{steps.map(([label,path],i)=><React.Fragment key={path}><div className={i<=active?"on":""} role="button" tabIndex={0} aria-label={`Go to ${label}`} onClick={()=>go(i,path)} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();go(i,path);}}}><i>{i<active?<Check size={12}/>:i+1}</i><span>{label}</span></div>{i<4&&<b/>}</React.Fragment>)}</div>; }
