    ...
    <div data-testid={`property-card-${p.id}`}
      ...
      <Link to={`/property/${p.id}`} className="block">
        <div className="relative aspect-[4/3] overflow-hidden bg-sand-100">
          {p.images?.[0] ? <img src={p.images[0]} alt={p.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /> : <div className="w-full h-full grid place-items-center text-sm text-muted-foreground">No property image available</div>}
          <PropertyBadges p={p} />
        ...
        <div className="px-5 pt-5 pb-2">
          ...
          <h3 className="font-serif text-xl leading-snug text-ink-900 line-clamp-2 group-hover:text-pine-500">
            {p.property_reference ? `${p.property_reference} — ` : ""}{p.title}
          </h3>
        </div>
      </Link>
